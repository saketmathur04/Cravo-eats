...
/**
 * Creates a new customer order from the active cart.
 * 
 * - Calculates geolocation distance between the customer and restaurant.
 * - Computes total order amounts (subtotal, platform fee, dynamic delivery fee).
 * - Creates an initial 'placed' order and triggers a realtime socket event.
 */
...
const ALLOWED_STATUSES = ["accepted", "preparing", "ready_for_rider", "cancelled"] as const;
...
export const updateOrderStatus = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    if (order.deliveryAddress && !order.deliveryAddress.formattedAddress) {
      order.deliveryAddress.formattedAddress = "Legacy Address";
    }
